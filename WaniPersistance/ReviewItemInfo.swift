...
class ReviewItemInfo: Object {
  ...
  typealias PersistantType = WaniPersistance.ReviewItemInfo
  typealias WaniType = WaniModel.ReviewItemInfo

  dynamic var radical: WaniPersistance.RadicalInfo?
  dynamic var kanji: WaniPersistance.KanjiInfo?
  dynamic var word: WaniPersistance.WordInfo?
...
extension ReviewItemInfo.WaniType: PersistanceModelInstantiatible {
  ...
  init(realmObject: PersistantType) {
    ...
    switch item {
      case let item as WaniPersistance.KanjiInfo: self = .kanji(item.waniModelStruct)
      case let item as WaniPersistance.WordInfo: self = .word(item.waniModelStruct)
      case let item as WaniPersistance.RadicalInfo: self = .radical(item.waniModelStruct)
      default: fatalError()
    }
  }
}
